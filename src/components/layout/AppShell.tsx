...
const AppShell: React.FC<{ children: React.ReactNode; title: string }> = ({ children, title }) => {
  ...
  return (
    <div className="page">
      {/* Top bar — padding-top adds the iPhone status-bar safe area so the menu/
          user-switch buttons aren't hidden under the notch/dynamic island when
          the app is opened from a Home Screen shortcut. */}
      <div style={{
